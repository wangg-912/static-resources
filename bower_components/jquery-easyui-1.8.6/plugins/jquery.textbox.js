...
if(_9.multiline){
$("<label class='textbox-text-label' for=\"+_a+\"></label><textarea id="+_a+" class='textbox-text' autocomplete='off'></textarea>").prependTo(tb);
}else{
$("<label class='textbox-text-label' for=\"+_a+\"></label><input id=\""+_a+"\" type=\""+_9.type+"\" class=\"textbox-text\" autocomplete=\"off\">").prependTo(tb);
}
...
_15.label._size({width:_16.labelWidth=="auto"?tb.outerWidth():_16.labelWidth},tb);
                if (_16.height != "auto" && !_16.dynamic) {
...
var _1e=_16.height=="auto"?_19.outerHeight():(tb.height()-_1d("top")-_1d("bottom"));
        ...
        if(_16.multiline && _16.dynamic){
            _19.css({"overflow-y":"hidden"});
            var _1e= tb[0].offsetHeight;
        }
...
_19._outerWidth(_1c);
 if(_16.multiline && _16.dynamic){
            ...
            _19.unbind("keydown").keydown(function (event) {
                if(event.keyCode == 13){
                    _16.enterKey ++;
                    var _minh = parseInt(_16.minHeight),
                        _matchValue = _19.val().match(/\r\n/ig) || _19.val().match(/\n/ig)
                        _h = _matchValue? (_matchValue.length+1) *16 : 16,
                        _maxh = parseInt(_16.maxHeight);
                    if(_h > _minh && _h<_maxh){
                        _19.parent().css({"min-height":(_h+2)+"px",height: (_h+2) + "px"});
                        _19.css({"overflow-y":"hidden",height: _h + "px"});
                    }
                    if(_h >= _maxh){
                        if(isActived) {
                            _19.css({"overflow-y":"auto",height: _maxh + "px"});
                            _19.parent().css({"min-height":_maxh+"px",height: _maxh + "px"});
                            isActived = false;
                        }
                    }
                }
                if(event.keyCode==8){
                    var _minh = parseInt(_16.minHeight),
                        _maxh = parseInt(_16.maxHeight),
                        _dynamicVal =  _19.val(),
                        _dyMatchValue = _dynamicVal.match(/\r\n/ig) || _dynamicVal.match(/\n/ig);

                    if(_dyMatchValue){
                        var _height = _dyMatchValue.length *16;
                        if(_minh < _height && _maxh >_height){
                            _19.parent().css({"min-height":(_height+2)+"px",height: (_height+2) + "px"});
                            _19.css({"overflow-y":"hidden",height: _height + "px"});
                        }
                        if(_minh >= _height){
                            _19.parent().css({"min-height":(_minh+2)+"px",height: (_minh+2) + "px"});
                            _19.css({"overflow-y":"hidden",height: _minh + "px"});
                        }
                    }
...
if(_50.dynamic && _50.multiline){
                        var _minh = parseInt(_50.minHeight),
                            _maxh = parseInt(_50.maxHeight),
                            _dyMatchValue = _4f.match(/\r\n/ig) || _4f.match(/\n/ig);
                        if(_dyMatchValue){
                            var _height = _dyMatchValue.length *16;
                            if(_minh < _height && _maxh >_height){
                                _51.parent().css({"min-height":(_height+2)+"px",height: (_height+2) + "px"});
                                _51.css({"overflow-y":"hidden",height: _height + "px"});
                            }
                            if(_minh >= _height){
                                _51.parent().css({"min-height":(_minh+2)+"px",height: (_minh+2) + "px"});
                                _51.css({"overflow-y":"hidden",height: _minh + "px"});
                            }
                            if(_height >= _maxh){
                                _51.css({"overflow-y":"auto",height: _maxh + "px"});
                                _51.parent().css({"min-height":_maxh+"px",height: _maxh + "px"});
                            }
                        }
                    }
_51.val(_4f);
